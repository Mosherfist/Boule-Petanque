...
def datenbank_erstellen():
    ...
    if os.path.exists(datenbank_name + ".db"):
        ...
        c = conn.cursor()
        c.execute("CREATE TABLE Teilnehmer (Team_ID INTEGER Primary Key, Teamname TEXT, Aktiv INTEGER, Freilos INTEGER, gewonnene_Spiele INTEGER, Punkte INTEGER)")
...
def teilnehmer_eingeben(datenbank_name):
    ...
    if Teamname == "q":
        ...
        if entscheidung == "j":
            conn = sqlite3.connect(datenbank_name + ".db")
            c = conn.cursor()
            c.execute("""INSERT into Teilnehmer (Teamname, Aktiv, Freilos, gewonnene_Spiele, Punkte) Values(?, ?, ?, ?, ?)""", (Teamname, 0, 0, 0, 0))
            conn.commit()
...
def turnier_auslosung(datenbank_name):
    ...
    for i in range(len(teilnehmer_liste)):
        c.execute("""INSERT into Spielablauf (Spiel_ID, Runde, Gegner_1, Gegner_2) Values(?, 1, ?, ?)""", (100 + counter, teilnehmer_liste[counter], teilnehmer_liste2[counter]))
        conn.commit()
        counter += 1
